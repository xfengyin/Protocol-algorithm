...
class TestLEACHVariants:
    ...
    @pytest.mark.parametrize("protocol_name", ["leach", "leach_c", "leach_ee", "leach_m"])
...
class TestEdgeCases:
    ...
    def test_single_node_network(self):
        """单节点网络"""
        network = Network(n_nodes=1, area=(0, 100, 0, 100), base_station_pos=(50, 50), seed=42)
    ...
    @pytest.mark.xfail(
        strict=True,
        reason=(
            "已知缺陷：其他节点全部死亡后 cluster_heads 归 0，剩余普通节点无簇头可加入，"
            "收发为 0 导致能耗恒为 0，进入零能耗僵死状态，n_alive 永远 >= 1（实测跑 1000 轮仍存活，"
            "能量稳定停在 1.9e-05）。修复需调整无簇头时的基础电路能耗语义，见 issue。"
        ),
    )
...
class TestFeatureEngineering:
    ...
    def test_feature_normalization(self):
        ...
        # 真实特征矩阵含退化列（方差为 0），仅对非常数列断言标准化性质
        varying = np.std(features, axis=0) > 1e-9
        col_means = np.mean(normalized[:, varying], axis=0)
        col_stds = np.std(normalized[:, varying], axis=0)
