...
class Network:
    ...
    def get_neighbors(self, node: Node, threshold_distance: float) -> List[Node]:
        ...
        idx_list = self._spatial_index.query_ball_point((node.x, node.y), threshold_distance)

        return [self.alive_nodes[i] for i in idx_list if self.alive_nodes[i].id != node.id]

    def get_neighbors_vectorized(self, threshold_distance: float) -> Dict[int, List[int]]:
        """
        批量获取所有节点的邻居（向量化版本）
    ...
    def steady_phase(self, data_size: int = 4000) -> None:
        ...
        for i, node in enumerate(alive_nodes_list):
            ...
            tx_energy = self.energy_model.calc_transmit_energy(min_distances[i], data_size)
        ...
        for node in self.alive_nodes:
            if not node.is_cluster_head:
                ...
                if nearest_ch:
                    ...
                    tx_energy = self.energy_model.calc_transmit_energy(min_dist, data_size)
                    node.consume_energy(tx_energy)
    ...
    def simulate_network(
        ...
        stop_condition: Optional[Callable[[Any, int], bool]] = None,
        **kwargs,
    ) -> Dict[str, Any]:
        """
        模拟整个网络生命周期
