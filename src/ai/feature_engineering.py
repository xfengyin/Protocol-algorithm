...
class AdvancedFeatureExtractor:
    ...
    def __init__(self, network: Network, n_bins: int = 10):
        ...
        self._energy_ranks: Optional[Dict[int, float]] = None
        self._neighbor_cache: Dict[Tuple[int, float], List[Node]] = {}
    ...
    def extract_features(self, node: Node) -> np.ndarray:
        ...
        dist_to_center = np.sqrt((node.x - stats.center_x) ** 2 + (node.y - stats.center_y) ** 2)
        ...
        features["neighbor_count_20"] = len(self._get_neighbors_cached(node, 20))
        features["neighbor_count_40"] = len(self._get_neighbors_cached(node, 40))
        features["neighbor_count_60"] = len(self._get_neighbors_cached(node, 60))
    ...
    def _get_neighbors_cached(self, node: Node, radius: float) -> List[Node]:
        """获取邻居（带缓存）"""
        cache_key = (node.id, radius)

        if cache_key not in self._neighbor_cache:
            self._neighbor_cache[cache_key] = self.network.get_neighbors(node, radius)
    ...
    def _compute_energy_ranks(self) -> None:
        ...
        sorted_energies = sorted(
            [(n.id, n.energy) for n in alive], key=lambda x: x[1], reverse=True
        )

        n = len(sorted_energies)
        self._energy_ranks = {
            node_id: (n - rank) / n for rank, (node_id, _) in enumerate(sorted_energies)
        }
    ...
    def _get_avg_neighbor_energy(self, node: Node, radius: float) -> float:
        ...
        return float(np.mean([n.energy for n in neighbors]))
    ...
    def apply_weights(
        self, features: np.ndarray, weights: Optional[Dict[str, float]] = None
    ) -> np.ndarray:
        """
        应用特征权重
...
class FeatureSelector:
    ...
    def select_by_correlation(
        self, features: np.ndarray, threshold: float = 0.95
    ) -> Tuple[np.ndarray, List[int]]:
        """
        基于相关性的特征选择
