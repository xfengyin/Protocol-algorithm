...
class OptimizedNetworkAnimator:
    ...
    def _setup_artists(self) -> None:
        ...
        self.scatter_cluster_heads = self.ax.scatter(
            ...
            s=self.config.cluster_head_size,
            marker="*",
            zorder=5,
        )
    ...
    def animate_to_list(
        self, rounds: int, protocol_name: str = "leach", **kwargs
    ) -> List[Dict[str, Any]]:
        """
        预计算所有帧数据（用于快速预览）
        ...
        for r in range(rounds):
            ...
            frame_data = {
                ...
                "dead_nodes": [{"id": n.id, "x": n.x, "y": n.y} for n in self.network.dead_nodes],
                "cluster_heads": [
                    {
                        "id": ch.node.id,
                        "x": ch.node.x,
    ...
    def create_comparison_animation(
        ...
        def update(frame):
            for i, (network, sc_alive, sc_dead, sc_ch, txt) in enumerate(
                ...
                sc_ch.set_offsets([(ch.node.x, ch.node.y) for ch in network.cluster_heads])

                txt.set_text(f"R:{network.current_round} A:{len(alive)}")

            return sum(
                [[s, t] for s, t in zip(scatters_alive + scatters_dead + scatters_ch, texts)],
                [],
            )

        return animation.FuncAnimation(fig, update, frames=rounds, interval=200, blit=False)
