...
class ParallelSimulationEngine:
    ...
    def run_parameter_sweep(
        ...
        param_values = tuple(tuple(v) for v in param_grid.values())
    ...
    def run_parallel_experiments(
        self, configs: List[SimulationConfig], show_progress: bool = True
    ...
    def _aggregate_results(
        ...
        for params in itertools.product(*param_values):
            ...
            matching = [
                r
                for r in results
                if all(getattr(r.config, name, None) == value for name, value in param_dict.items())
            ]
...
def _run_single_simulation(
    ...
    results = network.simulate_network(rounds=rounds, protocol_name=protocol_name)
...
class BatchSimulator:
    ...
    def compare_protocols(
        ...
        comparison: Dict[str, Any] = {"protocols": {}, "summary": {}}
        ...
        for protocol in protocols:
            ...
            results = self.engine.run_parameter_sweep(
                base_config=config, param_grid={}, n_runs=n_runs, aggregate=True
            )

            comparison["protocols"][protocol] = results

        all_lifetimes = {
            p: comparison["protocols"][p]["summary"].get("network_lifetime", 0) for p in protocols
        }
